...
// TODO: Set your Mailchimp sender address(es) before enabling MAILCHIMP_TEST_WARMER
      ...
      try {
        ...
        if (!decisionRow.open_done) {
          ...
          if (openWriteOk) {
            if (trackingPixels.length > 0) {
              console.log(`          📊 Found ${trackingPixels.length} Mailchimp tracking pixel(s)`);
        ...
        if (ENABLE_CLICKS && willClick && !decisionRow.click_done) {
          ...
          if (clickWriteOk) {
            ...
            // Prioritize Mailchimp click tracking links
            const mailchimpClickLinks = clickableLinks.filter(url => {
              const lowerUrl = url.toLowerCase();
              return lowerUrl.includes('mailchi.mp') ||
                (lowerUrl.includes('list-manage.com') && lowerUrl.includes('track/click'));
            });
            const linksToClickFrom = mailchimpClickLinks.length > 0 ? mailchimpClickLinks : clickableLinks;
            if (mailchimpClickLinks.length > 0) console.log(`          🎯 Found ${mailchimpClickLinks.length} Mailchimp click link(s)`);
...
const engageTestMailchimp = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [MAILCHIMP] TEST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[MAILCHIMP] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageTestMailchimp };

if (require.main === module) {
  engageTestMailchimp().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
