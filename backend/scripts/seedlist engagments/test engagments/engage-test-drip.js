...
const TARGET_SENDERS = [
  'mitchell.jennifer@nicegoldenfoods.com',
];
    ...
    for (const email of emails) {
      try {
        ...
        if (trackingPixels.length > 0) {
          console.log(`          📊 Found ${trackingPixels.length} Drip tracking pixel(s)`);
        ...
        if (ENABLE_CLICKS && willClick) {
          ...
          if (dripClickLinks.length > 0) console.log(`          🎯 Found ${dripClickLinks.length} Drip click link(s)`);
...
const engageTestDrip = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [DRIP] TEST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[DRIP] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageTestDrip };

if (require.main === module) {
  engageTestDrip().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
