...
// TODO: Set your MailerLite sender address(es) before enabling MAILERLITE_TEST_WARMER
const TARGET_SENDERS = [
  // 'your-mailerlite-sender@example.com',
  'irene.liam@heyzenlytic.live'
];
...
const getOrCreateCampaign = async (campaignKey, senderEmail, subject, date, allMailboxes) => {
  ...
  const { data: campaign, error: insertError } = await supabase
    ...
    .insert({ campaign_key: campaignKey, provider: 'mailerlite', sender_email: senderEmail,
  ...
  try {
    ...
    for (const email of emails) {
      try {
        ...
        // ── MAILERLITE-ONLY tracking pixel detection ──────────────────
        // MailerLite open tracking: open.mailerlite.com or img with mailerlite.com
        // MailerLite click tracking: click.mailerlite.com
        const trackingPixels = allUrls.filter(url => {
          const lowerUrl = url.toLowerCase();
          const isMLTracker =
            (lowerUrl.includes('mailerlite.com') && !lowerUrl.includes('click.mailerlite.com')) ||
            (lowerUrl.includes('mlsend.com') && !lowerUrl.includes('click.mlsend.com'));
          return isMLTracker;
        });
        ...
        if (!decisionRow.open_done) {
          ...
          if (openWriteOk) {
            if (trackingPixels.length > 0) {
              console.log(`          📊 Found ${trackingPixels.length} MailerLite tracking pixel(s)`);
        ...
        if (ENABLE_CLICKS && willClick && !decisionRow.click_done) {
          ...
          if (clickWriteOk) {
            ...
            // Prioritize MailerLite click tracking links
            const mlClickLinks = clickableLinks.filter(url => {
              const lowerUrl = url.toLowerCase();
              return lowerUrl.includes('click.mailerlite.com') || lowerUrl.includes('click.mlsend.com');
            });
            const linksToClickFrom = mlClickLinks.length > 0 ? mlClickLinks : clickableLinks;
            if (mlClickLinks.length > 0) console.log(`          🎯 Found ${mlClickLinks.length} MailerLite click link(s)`);
...
const engageTestMailerLite = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [MAILERLITE] TEST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[MAILERLITE] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageTestMailerLite };

if (require.main === module) {
  engageTestMailerLite().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
