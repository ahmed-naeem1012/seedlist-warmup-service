...
// TODO: Set your ActiveCampaign sender address(es) before enabling ACTIVECAMPAIGN_TEST_WARMER
const TARGET_SENDERS = [
  // 'your-activecampaign-sender@example.com',
  'damian.darko@procloudmira.live'
];
...
const getOrCreateCampaign = async (campaignKey, senderEmail, subject, date, allMailboxes) => {
  ...
  const { data: campaign, error: insertError } = await supabase
    ...
    .insert({ campaign_key: campaignKey, provider: 'activecampaign', sender_email: senderEmail,
  ...
  try {
    ...
    for (const email of emails) {
      try {
        ...
        // ── ACTIVECAMPAIGN-ONLY tracking pixel detection ──────────────
        // AC open tracking is a 1x1 img pixel at activehosted.com/lt.php
        const trackingPixels = allUrls.filter(url => {
          const lowerUrl = url.toLowerCase();
          return (
            (lowerUrl.includes('activehosted.com') && lowerUrl.includes('lt.php')) ||
            lowerUrl.includes('ac-track.com')
          );
        });
        ...
        if (!decisionRow.open_done) {
          ...
          if (openWriteOk) {
            if (trackingPixels.length > 0) {
              console.log(`          📊 Found ${trackingPixels.length} ActiveCampaign tracking pixel(s)`);
        ...
        if (ENABLE_CLICKS && willClick && !decisionRow.click_done) {
          ...
          if (clickWriteOk) {
            ...
            // Prioritize ActiveCampaign click tracking links
            const acClickLinks = clickableLinks.filter(url => {
              const lowerUrl = url.toLowerCase();
              return lowerUrl.includes('activehosted.com') || lowerUrl.includes('ac-track.com');
            });
            const linksToClickFrom = acClickLinks.length > 0 ? acClickLinks : clickableLinks;
            if (acClickLinks.length > 0) console.log(`          🎯 Found ${acClickLinks.length} AC click link(s)`);
...
const engageTestActiveCampaign = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [ACTIVECAMPAIGN] TEST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[ACTIVECAMPAIGN] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageTestActiveCampaign };

if (require.main === module) {
  engageTestActiveCampaign().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
