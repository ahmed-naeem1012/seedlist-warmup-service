...
// TODO: Set your MailerLite sender address(es) before enabling MAILERLITE_SEEDLIST_WARMER
    ...
    for (const email of emails) {
      try {
        ...
        if (ENABLE_CLICKS && willClick) {
          ...
          const mlClickLinks = clickableLinks.filter(url => url.toLowerCase().includes('click.mailerlite.com') || url.toLowerCase().includes('click.mlsend.com'));
...
const engageSeedlistMailerLite = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [MAILERLITE] SEEDLIST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[MAILERLITE SEEDLIST] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageSeedlistMailerLite };

if (require.main === module) {
  engageSeedlistMailerLite().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
