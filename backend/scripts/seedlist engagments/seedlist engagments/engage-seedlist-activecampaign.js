...
// TODO: Set your ActiveCampaign sender address(es) before enabling ACTIVECAMPAIGN_SEEDLIST_WARMER
    ...
    for (const email of emails) {
      try {
        ...
        if (ENABLE_CLICKS && willClick) {
          ...
          const acClickLinks = clickableLinks.filter(url => url.toLowerCase().includes('activehosted.com') || url.toLowerCase().includes('ac-track.com'));
...
const engageSeedlistActiveCampaign = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [ACTIVECAMPAIGN] SEEDLIST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[ACTIVECAMPAIGN SEEDLIST] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageSeedlistActiveCampaign };

if (require.main === module) {
  engageSeedlistActiveCampaign().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
