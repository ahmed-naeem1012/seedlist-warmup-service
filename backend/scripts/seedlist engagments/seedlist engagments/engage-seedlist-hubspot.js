...
// TODO: Set your HubSpot sender address(es) before enabling HUBSPOT_SEEDLIST_WARMER
...
const MAX_EMAIL_AGE_HOURS = 250;
const CLICK_ENABLED_UNTIL_INDEX = 800;
  ...
  try {
    ...
    for (const email of emails) {
      try {
        ...
        if (Math.random() >= ENGAGEMENT_CONFIG.open_rate) {
          await markEmailAsRead(mailbox, email.uid);
          try { await moveToMaxifyLabel(mailbox, email.uid); } catch (e) { }
          continue;
        }
        ...
        const trackingPixels = allUrls.filter(url => {
          ...
          const isHubSpotTracker =
            ...
            lowerUrl.includes('t.hubspot.com') || lowerUrl.includes('t.sidekickopen') ||
            lowerUrl.includes('hubspotemail.net') || lowerUrl.includes('hubspotemail-eu1.net') ||
            lowerUrl.includes('hubspotfree.net') || lowerUrl.includes('hs-email.net') ||
            ...
            lowerUrl.includes('hubfs') || lowerUrl.includes('hs-scripts.com');
        ...
        if (ENABLE_CLICKS && willClick) {
          ...
          const hubspotClickLinks = clickableLinks.filter(url => {
            const lowerUrl = url.toLowerCase();
            return (lowerUrl.includes('hubspotlinks') && lowerUrl.includes('/ctc/')) || lowerUrl.includes('/e2t/c/') || lowerUrl.includes('/e2t/ct/');
          });
          ...
          if (linksToClickFrom.length > 0) {
            ...
            if (clicked) { clickedCount++; await new Promise(resolve => setTimeout(resolve, 2000)); }
          }
        }
...
const engageSeedlistHubspot = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [HUBSPOT] SEEDLIST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(` Found ${mailboxes.length} active mailboxes\n`);
    ...
    console.log(`\n[HUBSPOT SEEDLIST] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageSeedlistHubspot };

if (require.main === module) {
  engageSeedlistHubspot().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
