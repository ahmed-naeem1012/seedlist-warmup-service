...
// TODO: Set your Drip sender address(es) before enabling DRIP_SEEDLIST_WARMER
const TARGET_SENDERS = [
  // 'your-drip-sender@example.com',
];
...
const loadUrl = async (url, type = 'unknown', retries = 2) => {
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (page) {
        try { await Promise.race([page.close().catch(() => {}), new Promise(resolve => setTimeout(resolve, 2000))]); }
        catch (e) { }
      }
      releaseSemaphore();
  ...
  try {
    ...
    for (const email of emails) {
      try {
        ...
        // ── DRIP-ONLY tracking pixel detection ───────────────────────
        const trackingPixels = allUrls.filter(url => {
          const lowerUrl = url.toLowerCase();
          return (
            (lowerUrl.includes('getdrip.com') && !lowerUrl.includes('/click')) ||
            (lowerUrl.includes('links.drip.com') && !lowerUrl.includes('/click'))
          );
        });
        ...
        if (ENABLE_CLICKS && willClick) {
          ...
          const dripClickLinks = clickableLinks.filter(url => {
            const lowerUrl = url.toLowerCase();
            return (lowerUrl.includes('getdrip.com') && lowerUrl.includes('/click')) ||
              (lowerUrl.includes('links.drip.com') && lowerUrl.includes('/click'));
          });
          const linksToClickFrom = dripClickLinks.length > 0 ? dripClickLinks : clickableLinks;
...
const engageSeedlistDrip = async () => {
  const startTime = Date.now();
  console.log('\n========================================');
  console.log('   [DRIP] SEEDLIST ENGAGEMENT');
  console.log('========================================\n');
  ...
  try {
    ...
    console.log(`\n[DRIP SEEDLIST] Found: ${totalFound} | Opened: ${totalOpened} | Clicked: ${totalClicked} | ${duration}s\n`);
...
module.exports = { engageSeedlistDrip };

if (require.main === module) {
  engageSeedlistDrip().then(() => process.exit(0)).catch((error) => { console.error(error); process.exit(1); });
}
